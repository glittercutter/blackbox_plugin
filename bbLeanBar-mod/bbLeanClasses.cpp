/*
 ============================================================================
  ...
  Copyright @ 2011 Sebastien Raymond (workspace list)
...
//===========================================================================
// workspaceentry
#define WORKSPACE_ENTRY_MIN_WIDTH 18

class workspaceentry : public baritem
{
public:
	int m_index;
	int m_wsnum;
    ...
    //-----------------------------
    workspaceentry(int index, barinfo *bi, int wsnum) 
	:	baritem(M_WORKSPACE, bi), m_index(index), m_wsnum(wsnum)
    {
		m_text = getWorkspaceName(wsnum);
    }
    ...
    void draw()
    {
		StyleItem *pSI = (StyleItem*)GetSettingPtr(SN_TOOLBARLABEL);
        m_bar->pBuff->MakeStyleGradient(m_bar->hdcPaint, &mr, pSI, pSI->bordered);
		
		DesktopInfo DI;
		GetDesktopInfo(&DI);
		if (DI.number == m_wsnum)
			CreateBorder(m_bar->hdcPaint, &mr, ((StyleItem*)GetSettingPtr(SN_TOOLBARLABEL))->TextColor, 1);
	...
	int getTextWidth()
	{
		return imax((m_bar->get_text_width(m_text) + (((StyleItem*)GetSettingPtr(SN_TOOLBARLABEL))->marginWidth * 2)), WORKSPACE_ENTRY_MIN_WIDTH);
	}

	char* getWorkspaceName(int wsnum)
	{
		DesktopInfo DI;
		GetDesktopInfo(&DI);
    ...
    void mouse_event(int mx, int my, int message, unsigned flags)
    {
		...
		{
			PostMessage(BBhwnd, BB_WORKSPACE, (WPARAM)BBWS_SWITCHTODESK, (LPARAM)m_wsnum);
			return;
		}
		if (WM_RBUTTONUP == message)
		{
...
class workspaceitemlist : public baritemlist
{
    int len;
	bool m_wstate[MAX_WORKSPACE];
	int width;

public:
    workspaceitemlist(barinfo *bi)
	:	baritemlist(M_WORKSPACELIST, bi), len(0)
    {
	}

    //-----------------------------
    // This one assigns the individual locations and sizes for
    // the items in the workspace-list
    bool calc_sizes(void)
    {
		struct itemlist *p;
        bool f = false;
		int s = ((mr.bottom - mr.top) - m_bar->TRAY_ICON_SIZE)/2;
		int xpos = mr.left;
        int n = 0;
        dolist (p, items)
        {
            int right = xpos + ((workspaceentry*)p->item)->getTextWidth();
            if (right > mr.right)
                break;
	...
	int update()
	{
		...
		for (int i = 0; i < ts; i++)
		{
			int taskws = GetTaskWorkspace(GetTask(i));
			if (ws[taskws] == false)
			{
				++len;
				ws[taskws] = true;
		...
		for (int i = 0; i < MAX_WORKSPACE; i++)
		{
			...
			if (ws[i] != m_wstate[i])
			{
				...
				for (int i = 0; i < MAX_WORKSPACE; i++)
					if (ws[i] == true)
					{
						workspaceentry *wsentry = new workspaceentry(++n, m_bar, i);
						add(wsentry);
						width += wsentry->getTextWidth();
					}
				// save current state
	...
	void invalidate (int flag)
    {
        if (flag == M_WORKSPACELIST)
            baritemlist::invalidate(M_WORKSPACE);
        else
            baritem::invalidate(0);
    }
...
public:
    ...
    void invalidate(int flag)
    {
        if (M_TASKLIST == flag || M_TRAYLIST == flag || M_WINL == flag || M_WORKSPACELIST == flag)
        {
    ...
    void create_bar()
    {
        ...
        for (item_ptr = m_bar->item_string; *item_ptr;  item_ptr++)
        {
            ...
            case M_TRAY:
                ...
                break;
            case M_WORKSPACE:
                m_bar->m_workspaceitemlist = new workspaceitemlist(m_bar);
				add(m_bar->m_workspaceitemlist);
                break;
    ...
    bool calc_sizes(void)
    {
		...
		if (m_bar->m_workspaceitemlist != 0)
			workspacezone_width = m_bar->m_workspaceitemlist->update();
    ...
    bool calc_line_size(struct itemlist **p0, int top, int bottom, int height)
    {
        ...
        dolist (p, *p0)
        {
            ...
            case M_WORKSPACELIST:
                xpos += workspacezone_width;
                break;
            }
          t0 = t1;
        }
