...
struct barinfo : plugin_info
{
	...
	bool has_tray;
    bool has_tasks;
	class workspaceitemlist *m_workspaceitemlist;
...
enum cfg_types { R_BOL = 0, R_INT, R_STR, R_COL} ;
ST bool nobool;
...
LRESULT barinfo::wnd_proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam, LRESULT *ret)
{
    ...
    {       
        ...
        case BB_DESKTOPINFO:
            ...
            {
                set_screen_info();
				if (this->m_workspaceitemlist != 0)
					this->update(M_WORKSPACELIST);
                this->NewTasklist();
                update(UPD_DRAW);
            }
            break;
        ...
        case BB_TASKSUPDATE:
            ...
            if (TASKITEM_MODIFIED == lParam)
            {
                if (this->has_tasks)
					...
					// TODO update workspace list here too
            } else {
                this->update(UPD_DRAW);
            }
...
void barinfo::GetRCSettings()
{
    ...
    *ip = 0;
	...
	m_workspaceitemlist = 0;
// 	has_workspacelist = NULL != strchr(item_string, M_WORKSPACE);
}
